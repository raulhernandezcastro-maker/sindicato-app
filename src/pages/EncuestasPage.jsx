...
import { ClipboardList, CheckCircle, ChevronRight, ChevronLeft, Lock } from 'lucide-react'
...
export default function EncuestasPage() {
  ...
  const [paso, setPaso]                 = useState(0) // índice pregunta actual
  ...
  if (yaRespondio || enviado) return (
    <div className="flex flex-col items-center justify-center py-24 text-center px-4 max-w-md mx-auto">
      <div className="w-20 h-20 rounded-full flex items-center justify-center mb-4"
        ...
        <CheckCircle className="w-10 h-10" style={{ color: '#2d7a4f' }} />
      </div>
      <h2 className="text-xl font-bold mb-2" style={{ color: '#1e3a2f' }}>
        {enviado ? '¡Gracias por participar!' : 'Ya respondiste esta encuesta'}
      </h2>
      <p className="text-sm text-muted-foreground leading-relaxed">
        {enviado
          ? 'Tu respuesta ha sido registrada de forma anónima y confidencial. Tu opinión es muy importante para el Sindicato.'
  ...
  return (
    <div className="max-w-xl mx-auto px-4 pb-8">
      ...
      {/* Encabezado */}
      <div className="rounded-xl px-4 py-3 mb-4 flex items-center gap-3" style={{ backgroundColor: '#1e3a2f' }}>
        ...
        <div>
          ...
          {encuesta.descripcion && <p className="text-xs mt-0.5" style={{ color: '#a8d5b5' }}>{encuesta.descripcion}</p>}
        </div>
      </div>
      ...
      <div className="mb-6">
        ...
        <div className="w-full rounded-full h-2" style={{ backgroundColor: '#e5e7eb' }}>
          <div className="h-2 rounded-full transition-all" style={{ width: `${progreso}%`, backgroundColor: '#2d7a4f' }} />
        </div>
      </div>
      ...
      {preguntaActual && (
        <div className="rounded-xl border p-5 mb-4" style={{ borderColor: '#ddd6cc', backgroundColor: '#fff' }}>
          <p className="font-semibold text-base mb-1" style={{ color: '#1e3a2f' }}>
          ...
          {preguntaActual.tipo === 'escala' && (
            <div className="mt-3">
              <div className="flex justify-between gap-2">
                {ESCALA.map(n => (
                  <button key={n} onClick={() => setRespuesta(preguntaActual.id, n)}
                    ...
                    style={{
                      borderColor: respuestas[preguntaActual.id] === n ? '#2d7a4f' : '#e5e7eb',
                      backgroundColor: respuestas[preguntaActual.id] === n ? '#2d7a4f' : '#fff',
              ...
              {respuestas[preguntaActual.id] && (
                <p className="text-center text-sm font-medium mt-2" style={{ color: '#2d7a4f' }}>
            ...
            <div className="flex gap-3 mt-3">
              {['Sí', 'No'].map(op => (
                <button key={op} onClick={() => setRespuesta(preguntaActual.id, op)}
                  ...
                  style={{
                    borderColor: respuestas[preguntaActual.id] === op ? '#2d7a4f' : '#e5e7eb',
                    backgroundColor: respuestas[preguntaActual.id] === op ? '#2d7a4f' : '#fff',
            ...
            <div className="flex flex-col gap-2 mt-3">
              {preguntaActual.opciones.map((op, i) => (
                <button key={i} onClick={() => setRespuesta(preguntaActual.id, op)}
                  ...
                  style={{
                    borderColor: respuestas[preguntaActual.id] === op ? '#2d7a4f' : '#e5e7eb',
                    backgroundColor: respuestas[preguntaActual.id] === op ? '#f0f8f3' : '#fff',
                    color: respuestas[preguntaActual.id] === op ? '#1e3a2f' : '#555',
      ...
      <div className="flex gap-3">
        ...
        {paso < totalPreguntas - 1 ? (
          <button onClick={() => { if (puedeAvanzar()) setPaso(p => p + 1) }}
            ...
            style={{
              backgroundColor: puedeAvanzar() ? '#2d7a4f' : '#ccc',
              color: '#fff',
          ...
          <button onClick={handleEnviar} disabled={enviando || !puedeAvanzar()}
            ...
            style={{
              backgroundColor: puedeAvanzar() ? '#1e3a2f' : '#ccc',
              color: '#fff',
              cursor: puedeAvanzar() ? 'pointer' : 'not-allowed'
            }}>
