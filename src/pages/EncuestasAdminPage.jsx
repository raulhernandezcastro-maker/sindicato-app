...
import { Plus, Trash2, BarChart2, ClipboardList, Eye, EyeOff, RefreshCw, CheckCircle, XCircle, GripVertical } from 'lucide-react'
...
export default function EncuestasAdminPage() {
  ...
  const load = useCallback(async () => {
    ...
    setEncuestas(data || [])
    const { data: config } = await supabase.from('configuracion').select('valor').eq('clave', 'encuestas_habilitadas').single()
  ...
  const cambiarEstado = async (id, nuevoEstado) => {
    // Si se activa esta, cerrar las demás activas
    ...
    <div className="max-w-2xl mx-auto space-y-4">
      <div className="flex items-center gap-3 px-4 py-3 rounded-lg" style={{ backgroundColor: '#1e3a2f' }}>
        ...
        <div className="flex-1">
          ...
          <p className="text-xs" style={{ color: '#a8d5b5' }}>{encuestaSeleccionada?.titulo}</p>
        </div>
      ...
      <div className="rounded-xl border p-4 text-center" style={{ borderColor: '#ddd6cc' }}>
        <p className="text-3xl font-bold" style={{ color: '#1e3a2f' }}>{resultados.totalParticipantes}</p>
        ...
        <div className="space-y-4">
          {(resultados.preguntas || []).map((p, idx) => (
            <div key={p.id} className="rounded-xl border p-4" style={{ borderColor: '#ddd6cc', backgroundColor: '#fff' }}>
              <p className="font-semibold text-sm mb-3" style={{ color: '#1e3a2f' }}>{idx + 1}. {p.texto}</p>

              {p.tipo === 'escala' && (
                <div>
                  <p className="text-2xl font-bold mb-2" style={{ color: '#2d7a4f' }}>⌀ {p.stats.promedio}</p>
                    ...
                    {p.stats.distribucion.map(d => (
                      <div key={d.valor} className="flex items-center gap-2 text-xs">
                        ...
                        <div className="flex-1 rounded-full h-2" style={{ backgroundColor: '#e5e7eb' }}>
                          <div className="h-2 rounded-full" style={{ width: p.stats.total > 0 ? `${(d.count / p.stats.total) * 100}%` : '0%', backgroundColor: '#2d7a4f' }} />
                        </div>
                  ...
                  {Object.entries(p.stats.conteo || {}).map(([op, count]) => (
                    <div key={op} className="flex items-center gap-2 text-xs">
                      ...
                      <div className="w-24 rounded-full h-2" style={{ backgroundColor: '#e5e7eb' }}>
                        <div className="h-2 rounded-full" style={{ width: p.stats.total > 0 ? `${(count / p.stats.total) * 100}%` : '0%', backgroundColor: '#2d7a4f' }} />
                      </div>
  ...
  if (vistaActual === 'crear') return (
    <div className="max-w-2xl mx-auto space-y-4">
      ...
      <div className="rounded-xl border p-4 space-y-3" style={{ borderColor: '#ddd6cc' }}>
        <p className="font-semibold text-sm" style={{ color: '#1e3a2f' }}>Datos generales</p>
        <div>
      ...
      <div className="rounded-xl border p-4 space-y-3" style={{ borderColor: '#ddd6cc' }}>
        <div className="flex items-center justify-between">
          <p className="font-semibold text-sm" style={{ color: '#1e3a2f' }}>Preguntas ({preguntas.length})</p>
        ...
        {preguntas.length === 0 && (
          <p className="text-xs text-muted-foreground text-center py-4">
            {form.plantilla_base !== 'personalizada' ? 'Cargando plantilla...' : 'Agrega preguntas manualmente o selecciona una plantilla'}
          </p>
        )}

        {preguntas.map((p, idx) => (
          <div key={idx} className="p-3 rounded-lg border space-y-2" style={{ borderColor: '#e5e7eb', backgroundColor: '#fafafa' }}>
            <div className="flex items-center gap-2">
              <span className="text-xs font-bold w-5 text-center" style={{ color: '#2d7a4f' }}>{idx + 1}</span>
              ...
              <button onClick={() => eliminarPregunta(idx)}><Trash2 className="w-4 h-4 text-red-400" /></button>
            </div>
            <div className="flex items-center gap-3 ml-7">
              ...
              <label className="flex items-center gap-1 text-xs cursor-pointer">
                <input type="checkbox" checked={p.obligatoria} onChange={e => actualizarPregunta(idx, 'obligatoria', e.target.checked)} className="accent-green-700" />
                Obligatoria
              </label>
            </div>
            {p.tipo === 'opcion_multiple' && (
      ...
      {encuestas.length === 0 ? (
        ...
        <div className="space-y-3">
          {encuestas.map(enc => {
            ...
            return (
              <div key={enc.id} className="rounded-xl border p-4" style={{ borderColor: '#ddd6cc', backgroundColor: '#fff' }}>
                <div className="flex items-start justify-between gap-2 mb-2">
                  <div className="flex-1 min-w-0">
                    <p className="font-semibold text-sm truncate" style={{ color: '#1e3a2f' }}>{enc.titulo}</p>
                    <p className="text-xs text-muted-foreground">{PLANTILLAS[enc.plantilla_base]} · {VISIBLE_PARA[enc.visible_para]}</p>
                  </div>
